...
class HTML
{
  ...
  public static function datalist($id, array $options, $name = null, $value = null)
  {
    $h = '';
    foreach($options as $k => $v)
    {
    ...
    if($name) {
      $h = self::tag('select', $h, array('name'=>'_'.$name));
      $h = self::tag('label', 'Fallback: ' . $h);
    }

    return self::tag('datalist', $h, array('id' => $id));
  }
